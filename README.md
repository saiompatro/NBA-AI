...
- **Player shot chart** — half-court SVG of every shot attempt (make/miss, court location) plus a per-zone FGA/FGM/FG%/PTS-per-shot breakdown on every player profile, sourced live from `shotchartdetail` (playoffs first, regular-season fallback), with an All/Made/Miss toggle
...
| GET    | `/api/players/<id>/shot-chart`                                | Shot-location chart + per-zone efficiency for a player (playoffs, falling back to regular season) |
...
- **Endpoints used:**
  ...
  - `nba_api.stats.endpoints.shotchartdetail` — per-player shot locations for the shot chart
