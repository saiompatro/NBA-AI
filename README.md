...
- **Shot chart & zone profile** — real shot-location data (`shotchartdetail`) rendered as a half-court plot on every player profile, alongside a Cleaning-the-Glass-style zone table (Restricted Area / Paint / Mid-Range / Corner 3s / Above-the-Break 3) showing attempt frequency, FG%, league-average FG% for that zone, and the delta, with a plain-English "best zone" blurb
...
| GET    | `/api/players/<id>/shot-chart`                                | Real shot-location plot + zone efficiency table for a player (playoffs, falling back to regular season) |
...
- **Endpoints used:**
  ...
  - `nba_api.stats.endpoints.shotchartdetail` — real shot-location data for the player shot chart
