...
- **Shot chart + zone efficiency** — real shot locations (made/missed) plotted on an inline half-court, plus a per-zone FG% table benchmarked against the league average for that zone, sourced from `shotchartdetail` with a regular-season fallback
...
| GET    | `/api/players/<id>/shot-chart`                                | Real shot locations + per-zone FG% vs league average (playoffs, falling back to regular season) |
...
- **Endpoints used:**
  ...
  - `nba_api.stats.endpoints.shotchartdetail` — shot locations and league zone averages
