...
- **Player shot chart** — real made/missed shot locations on a half-court SVG, plus a zone table (Restricted Area, Paint, Mid-Range, Corner 3, Above the Break 3) with the player's FG% against the league average in that same zone, pulled live from `shotchartdetail`
...
| GET    | `/api/players/<id>/shot-chart`                                | Real shot locations + zone FG% vs. league average for a player (playoffs, falling back to regular season) |
...
- **Endpoints used:**
  ...
  - `nba_api.stats.endpoints.shotchartdetail` — per-player shot locations + league zone averages
