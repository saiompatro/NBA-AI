...
- **Scores page** — a date-navigable league scoreboard (every ESPN/NBA.com/CBS Sports competitor's front door, which this app never had): final/live/scheduled games for any date with per-quarter line scores, records, and a date strip to page backward/forward or jump straight to today
...
| GET    | `/api/scores?date=YYYY-MM-DD`                                 | Full-league scores for one date (defaults to today), with line scores |
...
| `#/scores`              | Date-navigable scoreboard |
