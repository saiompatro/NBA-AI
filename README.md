...
- **Player shot chart** — every real shot attempt (make + miss) for a player this season plotted on a half-court diagram, with an All/2PT/3PT filter and a per-zone FG% table (Restricted Area, Mid-Range, Corner 3s, Above the Break 3) versus league average, pulled live from `shotchartdetail`
...
| `#/shot-chart`           | Player shot chart     |
| `#/shot-chart/<slug>-<id>` | Player shot chart (deep link) |
...
- **Endpoints used:**
  ...
  - `nba_api.stats.endpoints.shotchartdetail` — per-player shot locations + league-average zone FG%
