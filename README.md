...
- **Shot chart** — real shot-location map and zone-by-zone FG%/points-per-shot vs. league average on every player profile, from NBA Stats `shotchartdetail` (playoffs with a regular-season fallback, and a deterministic zone-shape estimate when live shot data isn't available)
...
| GET    | `/api/players/<id>/shot-chart`                                | Shot-location map + zone-aggregated FG%/PPS vs. league average for a player |
...
- **Endpoints used:**
  ...
  - `nba_api.stats.endpoints.shotchartdetail` — player shot locations + league zone averages
