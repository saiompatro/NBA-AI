...
- **Shot chart** — SVG half-court visualization of every shot (make/miss, location) on each player profile, with All/2PT/3PT/Made-only filters and a per-zone FG% vs. league-average table, pulled live from `shotchartdetail` with the same playoffs-first/regular-season fallback
...
| GET    | `/api/players/<id>/shot-chart?season=2025-26`                | Per-shot locations + per-zone FG% vs. league average (playoffs, falling back to regular season) |
...
- **Endpoints used:**
  ...
  - `nba_api.stats.endpoints.shotchartdetail` — per-shot chart data + league shooting averages by zone
