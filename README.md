...
- **Shot chart** — SVG half-court scatter of a player's actual shot locations plus a zone efficiency table (FGA, FG%, PPS, shot share, and FG% vs. league average) for Restricted Area, Paint, Mid-Range, and the three 3-point zones, pulled live from `shotchartdetail` with the same playoffs-first/regular-season-fallback pattern as the game log
...
| GET    | `/api/players/<id>/shot-chart`                                | Shot locations + zone efficiency vs league average for a player (playoffs, falling back to regular season) |
...
- **Endpoints used:**
  ...
  - `nba_api.stats.endpoints.shotchartdetail` — per-player shot locations + league-average shot zones
