...
- **Player shot chart** — real charted FGA locations plotted on a half-court diagram (make/miss), with a per-zone volume/efficiency breakdown table, on every player profile (playoffs first, regular-season fallback), sourced from NBA Stats `shotchartdetail`
...
| GET    | `/api/players/<id>/shot-chart`                                | Charted FGA locations + per-zone FG% for a player (playoffs, falling back to regular season) |
...
- **Endpoints used:**
  ...
  - `nba_api.stats.endpoints.shotchartdetail` — per-player charted shot locations
