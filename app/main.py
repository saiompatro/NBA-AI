...
def create_app() -> Flask:
    ...
    @app.get("/api/players/<int:player_id>/game-log")
    ...
    def player_shot_chart(player_id: int):
        season = current_season()
        return jsonify(analytics.shot_chart(player_id, season))
    ...
    def standings():
        return jsonify(analytics.league_standings(current_season()))
    ...
    def scoreboard():
        ...
        try:
            game_date = datetime.strptime(raw_date, "%Y-%m-%d").date() if raw_date else date.today()
        except ValueError:
            game_date = date.today()
        return jsonify({"date": game_date.isoformat(), "games": analytics.scoreboard_for_date(game_date)})
